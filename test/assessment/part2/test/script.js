...
function createRiver(name, continent, lengthInKilometers) {
    var riversObject = {
        ...
        isLongerThan: function(x) {
            if (this.lengthInKilometers > x.lengthInKilometers) {
             return true; }
             else {
                 return false;
             }
...
var rivers = [createRiver("Amazon","South America",6575), createRiver("Danube","Europe",2850),
               createRiver("Ganges","Asia",2704), createRiver("Mekong","Asia",4350), 
               createRiver("Mississippi","North America",3730), createRiver("Nile","Africa",6650),
               createRiver("Volga","Europe",3530)];

function getRiverByName(checkProp) {
    for (var x=0; x < rivers.length; x++) {
    if (rivers[x].name === checkProp){
        return rivers[x];
    } 
}
};

function getRiversByContinent(checkProp) {
    var riversByContinent = [];
    for (var x=0; x < rivers.length; x++) {
        if (rivers[x].continent === checkProp){
            riversByContinent.push(rivers[x]);
        } 
    }
    return riversByContinent;
};

function getAverageLength () {
    var total=0;
    for (var x=0; x < rivers.length; x++) {
        total += rivers[x].lengthInKilometers;
    }
    return (total / rivers.length);
};